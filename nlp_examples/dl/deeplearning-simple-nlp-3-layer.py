...
class SentimentNetwork(object):
    ...
    def init_network(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        self.weights_layer_2 = np.random.normal(0.0, self.kaiming_initialization(self.hidden_nodes, self.hidden_nodes), (self.hidden_nodes, self.hidden_nodes))
        self.weights_layer_3 = np.random.normal(0.0, self.kaiming_initialization(self.hidden_nodes, self.output_nodes), (self.output_nodes, self.hidden_nodes))
        # layer_0是词频向量
    ...
    def train(self, training_reviews: DataFrame, training_label: DataFrame):
        ...
        for i in range(len(training_reviews)):
            ...
            layer_2_o = self.relu(layer_2_i)

            layer_3_i = np.dot(self.weights_layer_3, layer_2_o)
            ...
            # da^[3] = (1-y)/(1-a) - y/a                                          a^[l] (n^[l], 1)
            # dz^[3] = da^[3] * sigmoid_output_2_derivative(a^[3])                z^[l] (n^[l], 1)
            # dz^[3] = dz^[3] * sigmoid_output_2_derivative(a^[3])  这是逐元素相乘！！
            # dw^[3] = dz^[3] * a^[2].T. todo ???需要转秩 难道吴恩达课程上错了？       w^[l] (n^[l], n^[l-1])
            # db^[3] = dz^[3]                                                     b^[l] (n^[l], 1)
            # da^[2] = w^[3].T * dz^[3]

            da_3 = (1 - self.get_target_for_label(label)) / (1 - layer_3_o) - self.get_target_for_label(label) / layer_3_o
            dz_3 = da_3 * self.sigmoid_output_2_derivative(layer_3_o) # todo why？逐元素相乘
            print(f'da_3 shape: {da_3.shape}')
            print(f'dz_3 shape: {dz_3.shape}')
            print(f'layer_2_o shape: {layer_2_o.shape}')
            dw_3 = np.dot(dz_3, layer_2_o.T)
            db_3 = dz_3
            da_2 = np.dot(self.weights_layer_3.T, dz_3)
            print(f'dw_3 shape: {dw_3.shape}')
            print(f'db_3 shape: {db_3.shape}')
            print(f'da_2 shape: {da_2.shape}')
            print(f'weights_layer_3 shape: {self.weights_layer_3.shape}')
            self.weights_layer_3 -= self.learning_rate * dw_3


            # da^[2] = w^[3].T * dz^[3]
            # dz^[2] = da^[2] * relu_derivative(a^[2])  这是逐元素相乘！！！
            # dw^[2] = dz^[2] * a^[1].T
            # db^[2] = dz^[2]
            # da^[1] = w^[2].T * dz^[2]
            dz_2 = da_2 * self.relu_derivative(layer_2_o)
            ...
            # da^[1] = w^[2].T * dz^[2]
            # dz^[1] = da^[1] * relu_derivative(a^[1])。这是逐元素相乘！！！
            # dw^[1] = dz^[1] * a^[0].T
            # db^[1] = dz^[1]
            ...
            # 判断预测结果是否正确
            if layer_3_o >= 0.5 and label == 1:
                correct_so_far += 1
            elif layer_3_o < 0.5 and label == 0:
                correct_so_far += 1
